...
┛┗┛┗┛┃
    ...
    public partial class Form1 : Form
    {
        ...
        Animation sBullet, sShip, sShip_go, sRock, sRock_small, sExplosion;
        ...
        private void Init()
        {
            ...
            sRock = new Animation(new SpriteSheet(@"resource\rock.png", 64, 64), 30, 0, 15);
            sRock_small = new Animation(new SpriteSheet(@"resource\rock_small.png", 64, 64), 30, 0, 15);
            sExplosion = new Animation(new SpriteSheet(@"resource\spritesheet1.png", 100, 100), 90, 0, 73);


            // 初始化
            ship = new Ship(new Animation[] { sShip, sShip_go }, panel1.Width / 2, panel1.Height / 2, 20, 5);
        ...
        {
            // 計算FPS
            deltaTimeMs = Environment.TickCount - lastTime;
            timeAll.Add(1000.0 / (deltaTimeMs | 1));
            lastTime = Environment.TickCount;

            //--------------
            deltaTimeSec = (double)deltaTimeMs / 1000;
            update(deltaTimeSec);
            ...
            this.Text = $"{panel1.Size.ToString()}";
            if (deltaTimeMs >= delteT)
            {
                timer1.Interval = Math.Max((int)(delteT - (deltaTimeMs - delteT)), 1);
            }
        }
        private void update(double deltaTime)
        {
            ...
            for (int i = 0; i < entities.Count; i++)
            {
                for (int j = 0; j < entities.Count; j++)
                {
                    var entA = entities[i];
                    var entB = entities[j];
                    if (entA.name == "asteroid" && entB.name == "bullet")
                    {
                        if (Entity.isCollide(entA, entB))
                        {
                            entA.life -= 1;
                            entB.life -= 1;

                            Entity exp = new Entity(sExplosion, entA.pos.x, entA.pos.y, 1, 0, 0);
                            exp.name = "explosion";
                            entities.Add(exp);
                        }
                    }
        ...
        private void MakeAsteroid()
        {
            for (int i = 0; i < 15; i++)
            {
                entities.Add(new Asteroid(sRock, ran.Next(0, Width), ran.Next(0, Height), 20, ran.Next(20, 100), ran.NextDouble() * Math.PI));
            }
        }
