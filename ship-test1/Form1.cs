...
┛┗┛┗┛┃
    ...
    public partial class Form1 : Form
    {
        ...
        {
            // 計算FPS
            timeDiff = Environment.TickCount - lastTime;
            timeAll.Add(1000.0 / (timeDiff | 1));
            lastTime = Environment.TickCount;
        ...
        private void draw()
        {
            ...
            g.TranslateTransform((float)(ship.x), (float)ship.y);
            ...
            */
            ...
            g.DrawPolygon(Pens.Black, ship_points);// 如果覺得三角形有困難的話，可以先註解這行，用上面的圓形

            if (keys[1]) g.DrawLine(Pens.Red, -10, 0, -20, ran.Next(-8, 9));// 加速時的小特效
        ...
        private void DebugInf()
        {
            ...
            g_status = g.Save();
            g.TranslateTransform((float)ship.x, (float)ship.y);
            g.RotateTransform((float)(velocity.getAngle() * 180 / Math.PI));
            float len = (float)velocity.getLength() * 10;
            if (len > 1)
            {
                g.DrawLine(Pens.Orange, 0, 0, len, 0);
            ...
            g.DrawString($"position : {ship.ToString()}\r\n" +
